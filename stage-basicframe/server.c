...
void history_init(MSG *msg,char *buf)
{
	...
	{
		printf("%s.\n",errmsg);
		printf("insertnsert historyinfo failed.\n");
	}else{
		printf("insert historyinfo successfullycess.\n");
	}
}
...
int process_admin_modify_request(int acceptfd,MSG *msg)
{
	...
	case 'W':
		...
		sprintf(historybuf,"%s改工号为%d的职位为%s",msg->username,msg->info.no,msg->info.work);
		break;
	case 'T':
		sprintf(sql,"update usrinfo set date='%s' where staffno=%d;",msg->info.date, msg->info.no);
		sprintf(historybuf,"%s改工号为%d的入职日期为%s",msg->username,msg->info.no,msg->info.date);
		break;
	case 'L':
...
int process_admin_deluser_request(int acceptfd,MSG *msg)
{
	...
	if(sqlite3_exec(db,sql,NULL,NULL,&errmsg) != SQLITE_OK){
		...
		strcpy(msg->recvmsg,"update is ok!");
		send(acceptfd,msg,sizeof(MSG),0);
